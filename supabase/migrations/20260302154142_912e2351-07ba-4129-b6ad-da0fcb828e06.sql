...
-- Fix secure_complete_task to use dynamic daily cap (200/400) instead of hardcoded 180
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM weekend_campaigns WHERE is_active = true AND v_today BETWEEN start_date AND end_date
  ) INTO v_has_campaign;
  ...
  RETURN jsonb_build_object('success', true, 'message', 'Task completed!', 'points', v_points_to_award, 'task_title', v_task.title);
END;
$$;
